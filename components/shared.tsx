...
import { getAsset } from '@/data/assets/registry';

// Shared UI bits — placeholders, icons, brand mark

export const Mark = ({ showDescriptor = false }: { showDescriptor?: boolean }) => (
  <span
    className="nav-brand"
    style={{ flexDirection: showDescriptor ? 'column' : 'row', alignItems: showDescriptor ? 'flex-start' : 'center' }}
  >
    <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <span className="mark">N</span>
      <span>NEXS</span>
    </span>
    {showDescriptor && <span className="descriptor">ENGINEERED FOR PERFECT SURFACES</span>}
  </span>
);
...
export const ImgPh = ({ label, dark, ratio = '16 / 10', className = '', style, assetId }: ImgPhProps) => {
...
type IconProps = { name: string; size?: number; stroke?: number };

export const Icon = ({ name, size = 20, stroke = 1.4 }: IconProps) => {
  const p = {
    width: size,
    height: size,
    viewBox: '0 0 24 24',
...
export const QrGlyph = ({ size = 96 }: { size?: number }) => {
  const cells = [
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 15 15" shapeRendering="crispEdges">
      ...
      {([[0,0],[8,0],[0,8]] as const).map(([fx,fy], i) => (
        <g key={i}>
