...
import { DigitalWarrantySection, DealerInvite, QrProof, ContactStrip } from './home-sections';
import { BRAND, CTA, HOME_STATS, HOMEPAGE_PILLARS, PRODUCT_GROUPS, PRODUCTS, type Product } from '@/data/brand';
    ...
    <main data-screen-label="01 Home">
      <section className="section" style={{ paddingTop: 'clamp(56px,6vw,96px)' }}>
        <div className="container">
          ...
          <h1 className="h-display-lg" style={{ maxWidth: 980 }}>{BRAND.heroEN}</h1>
          <h2 className="h-2 thai" style={{ marginTop: 20, color: 'var(--nexs-ink-muted)', fontWeight: 400, maxWidth: 880 }}>
            {BRAND.heroTH}
          </h2>
          <p className="lede thai" style={{ maxWidth: 720, marginTop: 28 }}>{BRAND.heroSubcopyTH}</p>
          ...
          <div className="hero-stage" style={{ marginTop: 64, aspectRatio: '16 / 8', position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', right: 24, bottom: 24, color: 'var(--nexs-ink-soft)', fontSize: 11, fontFamily: 'var(--font-mono)' }}>001 / 04</div>
          </div>

          <div className="thai" style={{
            display: 'grid', gridTemplateColumns: 'repeat(4,1fr)',
            borderTop: '1px solid var(--nexs-hairline)', borderBottom: '1px solid var(--nexs-hairline)',
            ...
            {HOME_STATS.map((s, i) => (
              <div key={i} style={{ paddingLeft: i === 0 ? 0 : 24, borderLeft: i === 0 ? 'none' : '1px solid var(--nexs-hairline)' }}>
                ...
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 600, letterSpacing: '-0.02em', marginTop: 6 }}>{s.v}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="grid-3">
            <CategoryCard group={PRODUCT_GROUPS.clear} />
            <CategoryCard group={PRODUCT_GROUPS.matte} />
            <CategoryCard group={PRODUCT_GROUPS.color} />
          </div>
        ...
        <div className="container">
          <div className="grid-2" style={{ gap: 64, alignItems: 'center' }}>
            <div>
              ...
              <p className="lede thai" style={{ marginTop: 16 }}>
                NEXS พัฒนาเทคโนโลยีฟิล์มหลายชั้น เพื่อให้ผิวรถมีความใส เงา เรียบเนียน ทนทาน และมั่นใจได้ในการใช้งานระยะยาว
              </p>
...
export const CategoryCard = ({ group }: { group: typeof PRODUCT_GROUPS[keyof typeof PRODUCT_GROUPS] }) => {
  const go = useGo();
  return (
    <article className="card thai" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: 28, display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <p className="body" style={{ flex: 1 }}>{(group as { homepageTH: string }).homepageTH}</p>
        <div className="row" style={{ gap: 8, marginTop: 24 }}>
          <button className="btn btn-secondary btn-sm" onClick={() => go((group as { slug: string }).slug)}>View {group.name}</button>
...
export const ProductCard = ({ p }: { p: Product }) => {
  const go = useGo();
  const pill = (p as Product & { pill?: string }).pill;
  return (
    <article className="card thai" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div style={{
        aspectRatio: '4/3', position: 'relative',
        background: p.bg,
        display: 'grid', placeItems: 'end start', padding: 20,
      }}>
        ...
        <span className={`pill ${pill || ''}`} style={{ position: 'absolute', top: 16, left: 16 }}>
