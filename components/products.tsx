...
import { productsByGroup, PRODUCT_GROUPS, COMPARISON_ROWS_CLEAR, WHY_CLEAR_PPF, CTA } from '@/data/brand';
import { useGo } from '@/lib/use-go';

export const ClearPPFPage = () => {
  const go = useGo();
        ...
        <div className="container">
          <div className="grid-2" style={{ gap: 64, alignItems: 'center' }}>
            <div>
              ...
              <h1 className="h-1" style={{ marginTop: 14 }}>{(group as { heroTitle: string }).heroTitle}</h1>
              <p className="lede thai" style={{ marginTop: 20 }}>{(group as { heroTH: string }).heroTH}</p>
              <p className="thai" style={{ marginTop: 14, color: 'var(--nexs-ink-muted)', fontStyle: 'italic' }}>
                {(group as { heroConceptTH?: string }).heroConceptTH}
              </p>
...
type Row = { label: string; th: string; values: string[] };

export const ComparisonTable = ({ rows, columns }: { rows: Row[]; columns: string[] }) => {
  const colCount = columns.length;
  return (
    <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
      <div className="thai" style={{
        display: 'grid', gridTemplateColumns: `1.4fr repeat(${colCount}, 1fr)`,
        background: 'var(--nexs-canvas-soft)', padding: '22px 28px',
        ...
        <div key={i} className="thai" style={{
          ...
          {r.values.map((v, j) => (
            <div key={j} style={{
              ...
              fontWeight: v === '✓' ? 500 : 400, fontSize: 13,
            }}>
