...
import { Eyebrow, Icon, ImgPh, QrGlyph } from './shared.jsx';
import { ContactForm } from './contact-form.jsx';
import { CTA, BRAND } from '../data/brand.js';

// Home page — supporting sections (Digital Warranty, Dealer, QR proof, Contact strip)

export const DigitalWarrantySection = ({ go }) => (
          ...
          {[
            ...
            <div key={i} style={{display:"grid", gridTemplateColumns:"56px 1fr", gap:20, padding:"22px 0", borderTop: i===0?"1px solid var(--nexs-hairline)":"none", borderBottom:"1px solid var(--nexs-hairline)"}}>
              <div style={{
                ...
                fontFamily:"var(--font-mono)", fontSize:12, color:"var(--nexs-ink-soft)"
              }}>0{i+1}</div>
              <div>
...
export const DigitalCardMock = ({ data, state = "active" }) => {
  const d = data || {
    ...
    <div className="container">
      <div className="grid-2" style={{gap:64, alignItems:"center"}}>
        <div>
          ...
          <p className="lede thai" style={{marginTop:16}}>
            NEXS พร้อมสนับสนุนร้านติดตั้งด้วยระบบสินค้าที่ชัดเจน สื่อการขายที่พร้อมใช้งาน
            การอบรมมาตรฐาน และเครื่องมือที่ช่วยให้การแนะนำลูกค้าเป็นเรื่องง่ายขึ้น
          </p>
          ...
          <div className="row" style={{marginTop:32, gap:12}}>
            ...
            <button className="btn btn-ghost" onClick={()=>go("login")}>{CTA.dealerLogin}</button>
          </div>
        </div>
        <div>
