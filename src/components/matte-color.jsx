...
import { productsByGroup, PRODUCT_GROUPS, CTA } from '../data/brand.js';

// Matte PPF page — 3 tiers (Prime / Pro / Ultimate)
export const MattePPFPage = ({ go }) => {
...
// Color PPF page — 5 tiers
export const ColorPPFPage = ({ go }) => {
  const items = productsByGroup("color");
  const group = PRODUCT_GROUPS.color;
  // Swatch list — visual palette overview
  const swatches = [
    { name: "Red", c: "#D71920" },
    { name: "Blue", c: "#3B5A82" },
    { name: "Green", c: "#2E7D32" },
    { name: "Purple", c: "#6A1B9A" },
    { name: "Black", c: "#0E0E0F" },
    { name: "White", c: "#F2F3F4" },
    { name: "Carbon", c: "#1D1D1F" },
    { name: "Silver", c: "#BFC3C7" },
  ];
  return (
