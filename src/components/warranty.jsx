...
import { DigitalCardMock } from './home-sections.jsx';
import { Faq } from './faq.jsx';
import { CTA } from '../data/brand.js';

// Warranty pages: lookup + Digital Warranty Card landing (3 states)
// NEXS Differentiator — keep as public feature integrated into nav.

export const WarrantyPage = ({ go, navigateToCard }) => {
      ...
      <section className="page-hero">
        <div className="container">
          ...
          <p className="lede thai" style={{marginTop:20, maxWidth:680}}>
            กรอก Serial Number บนบรรจุภัณฑ์หรือบนบัตรรับประกันของคุณ
            เพื่อตรวจสอบสถานะบัตรรับประกันดิจิทัลของ NEXS
          </p>
        </div>
      </section>
      ...
      <section className="section-tight">
        <div className="container-narrow">
          <div className="card" style={{padding:40}}>
            <form className="thai" onSubmit={(e)=>{e.preventDefault(); navigateToCard(serial.trim() || "PRO-1196MXY0401178Q");}}>
              ...
              <div style={{marginTop:32, paddingTop:24, borderTop:"1px solid var(--nexs-hairline)"}}>
                <p className="body" style={{lineHeight:1.6}}>
                  <strong style={{color:"var(--nexs-ink)"}}>หมายเหตุสำคัญ:</strong> ลูกค้าไม่สามารถเปิดใช้งานบัตรรับประกันเองได้
                  การลงทะเบียนต้องให้ตัวแทนจำหน่ายหรือ Admin ของ NEXS เป็นผู้ลงทะเบียนหลังการติดตั้งจริง
                </p>
              </div>
            </form>
          </div>
          ...
          <div style={{marginTop:24, display:"flex", justifyContent:"center", gap:8, flexWrap:"wrap"}}>
            ...
            <button className="btn btn-ghost btn-sm" onClick={()=>navigateToCard("PRO-1196MXY0401178Q","active")}>Active</button>
            <button className="btn btn-ghost btn-sm" onClick={()=>navigateToCard("PRO-9999XYZ0401000A","not-registered")}>Found · ยังไม่ลงทะเบียน</button>
            <button className="btn btn-ghost btn-sm" onClick={()=>navigateToCard("INVALID-CODE","invalid")}>Invalid</button>
          </div>
        </div>
      </section>
...
// Digital Warranty Card landing — 3 states
export const WarrantyCardPage = ({ serial, state = "active", go }) => {
  return (
    <main data-screen-label={`13 Warranty Card · ${state}`} style={{background:"var(--nexs-canvas-soft)", minHeight:"100vh", padding:"48px 0 80px"}}>
      <div className="container-narrow">
        <button className="btn-link thai" onClick={()=>go("warranty")} style={{color:"var(--nexs-ink-muted)", marginBottom:24, fontSize:13}}>
...
const ActiveCard = ({ serial, go }) => (
  <>
    <DigitalCardMock data={{
      status:"Active", product:"NEXS Clear PPF · Pro", warrantyYears:10,
      serial: serial,
      vehicle:"Sedan · Pearl White",
      plate:"1กก ··3456",
      dealer:"NEXS Authorized · Bangkok",
      install:"12 มี.ค. 2026",
      expiry:"12 มี.ค. 2036",
    }} state="active"/>
...
const NotRegisteredCard = ({ serial, go }) => (
  <div className="card thai" style={{padding:40, textAlign:"center"}}>
    ...
    <p className="body" style={{maxWidth:520, margin:"16px auto 0"}}>
      Serial <span style={{fontFamily:"var(--font-mono)", color:"var(--nexs-ink)"}}>{serial}</span> ถูกผลิตและบันทึกในระบบ NEXS
    ...
    <div className="card-flat" style={{marginTop:32, textAlign:"left", padding:24, background:"var(--nexs-canvas-soft)", border:"none"}}>
      ...
      <ol style={{paddingLeft:18, marginTop:10, lineHeight:1.7, fontSize:14, color:"var(--nexs-ink-muted)"}}>
        ...
        <li>หากไม่สามารถติดต่อ dealer ได้ ส่งคำขอผ่าน "แจ้งบัตร / QR สูญหาย"</li>
      </ol>
    </div>
...
const InvalidCard = ({ serial, go }) => (
  <div className="card thai" style={{padding:40, textAlign:"center"}}>
    ...
    <p className="body" style={{maxWidth:520, margin:"16px auto 0"}}>
      <span style={{fontFamily:"var(--font-mono)", color:"var(--nexs-ink)"}}>{serial}</span> ไม่ปรากฏในระบบ
