...
import { ComparisonTable } from './products.jsx';
import { PRODUCTS, productsByGroup, CTA } from '../data/brand.js';

// Compare page — cross-category public matrix (Clear / Matte / Color group summary)
export const ComparePage = ({ go }) => {
  const [group, setGroup] = React.useState("clear");
      ...
      <section className="page-hero">
        <div className="container">
          ...
          <p className="lede thai" style={{marginTop:20, maxWidth:680}}>
            เปรียบเทียบรุ่นในแต่ละกลุ่มฟิล์ม เพื่อให้คุณเลือกได้ตามการใช้งานจริง
            สไตล์ที่ต้องการ และระดับพรีเมียมที่เหมาะกับรถของคุณ
          </p>
        </div>
      </section>
      ...
      <section style={{borderTop:"1px solid var(--nexs-hairline)"}}>
        <div className="container" style={{display:"flex", justifyContent:"center", padding:"20px 24px"}}>
          <div className="thai" style={{display:"inline-flex", gap:4, padding:4, borderRadius:999, background:"var(--nexs-canvas-soft)"}}>
            {[["clear","Clear PPF"],["matte","Matte PPF"],["color","Color PPF"]].map(([k,l])=>(
